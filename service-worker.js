"use strict";
var REQUIRED_CACHE = "unless-update-cache-v911-required";
var USEFUL_CACHE = "unless-update-cache-v911-useful";
var STATIC_CACHE = "unless-update-cache-v911-static";
...
var serve_cache = function (cache_origin, url){
    ...
    return cache_origin.then(function (cache) {
        ...
        return cache.match(url).then(function (response) {
            "use strict";
            return !response ? Promise.reject(): response.status === 200 ? response.clone()  || response: fetch(url).then(function (response2) { // Fetch, clone, and serve
                "use strict";
                if(response2.status === 200) { cache.put(url, response2.clone()); return Promise.resolve(response2.clone() || response2); } else { return Promise.reject(); }
            });
        }).catch(function(){
            "use strict";
            return fetch(url).then(function (response2) { // Fetch, clone, and serve
                "use strict";
                if(response2.status === 200) { cache.put(url, response2.clone()); return Promise.resolve(response2.clone() || response2); } else { return Promise.reject(); }
            });
        });
