...
class InnerToolbar extends React.Component {
    ...
    render() {
        ...
        const tip_items = [
            <Fade in={true}><a key="tip-legend" onClick={() => {this._trigger_tip(`*Aware it musts be working as intended by the user based on their results is knowing an artist in everyone. Yet one should get the fact that hints shown are usually most needed tools which are to be used from left to right in chronological order.`)}} className={classes.link}>&nbsp;Suggestions*&nbsp;›&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-deepai" onClick={() => {this._trigger_tip(`You can ask the default DeepAI.org system ("Image" tab (2nd) in the panel of editing, just below choosing the size you want) either using "COLORIZE" and "SCALE 2x" if ONLY you choose to not keep option "ORIGINAL" keeping it all usual on your device. DeepAI.org is using some noisy and resilient set of small packed opinions which passes into a kind of complex and blind musical-paper-reader about opinions reduced and stored knowing which results to tell for which colors that must get shape X or Y to paint into as long as they know the shape and what it means to them.`)}} className={classes.link}>&nbsp;AI?&nbsp;→&nbsp;</a></Fade>,
            ...
            <Fade in={true}><a key="tip-smooth" onClick={() => {this._trigger_tip(`For a pixel art to get smooth, your pixel should live with similar neighbors in towns that isn't fancy because some pixel are living the thug life within towns out of their id, based on a few two or more raids it is possible to move away many cheaters into thin air by comparing all-height other pixel's neighbors id and counting up to six similar id against the id of the pixel being judged within or outside of majority to execute its removal for being an artefact from a view based on their own district, algorithmic smoothing raids often results in nihilism adhesion (which that rejects generally fundamental concepts of human existence) as fewer outsiders last consequently on seen results due to all-good homogenous pixel's district being still just not in needs of Pyongyang authorities neither. Double-tap or Right-click around the movable and drawing card area to open the context-menu containing the related tools.`)}} className={classes.link}>&nbsp;+smooth&nbsp;→&nbsp;</a></Fade>,
