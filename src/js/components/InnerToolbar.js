...
class InnerToolbar extends React.Component {
    ...
    render() {
        ...
        const tip_items = [
            <Fade in={true}><a key="tip-legend" onClick={() => {this._trigger_tip(`(0?) Aware it musts be working as intended, tips are to be used left to right in chronological order while asterisks means mandatory for completion.`)}} className={classes.link}>&nbsp;0-5&nbsp;STEPS&nbsp;›&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-contrast" onClick={() => {this._trigger_tip("(1?) In photography, darkest and lightest tones looks better if they are more distant from each other. Double-tap or Right-click around the movable and drawing card area to open the context-menu containing the related tools.")}} className={classes.link}>&nbsp;1)&nbsp;~Contrasts&nbsp;→&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-colors" onClick={() => {this._trigger_tip("(2?) For a pixel art to get nice, your palette needs to contain less colors than 8-128 for greatness. Double-tap or Right-click around the movable and drawing card area to open the context-menu containing the related tools.")}} className={classes.link}>&nbsp;2)&nbsp;-Colours*&nbsp;→&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-smooth" onClick={() => {this._trigger_tip(`(3?) For a pixel art to get neat, you can raid the "smooth" functionality available in menu on double-tap or right-click. All your pixel should live with similar neighbors 6/8 because in towns that isn't thought "smooth or fancy" algorithmic raids will adopts neighbour's color for lonely pixel repainting.`)}} className={classes.link}>&nbsp;3)&nbsp;+Smoother&nbsp;→&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-filters" onClick={() => {this._trigger_tip(`(4?) In popular social media around pictures, it is popular to let colors being moved from given properties to other ones, creating sometimes colourful effects looking regularized. 7 Sections in the panel of editing contains one called "Filters" (7/7) to apply an adjustable color correction strength out of 21+ preview.`)}} className={classes.link}>&nbsp;4)&nbsp;+Filters&nbsp;→&nbsp;</a></Fade>,
            <Fade in={true}><a key="tip-export" onClick={() => {this._trigger_tip(`(5?) The nostalgia caused by legendary video games had let remastering find faithful solutions in saving fan's eyes on yet 2x or 3x up-scaled screen resolution using original visuals while up-scaled with intelligent rules only to cover just more precisely the same area while seeking beauty out of changes regarding pixel's neighbour tiny house. The 2nd panel section about "Images" let you export and multiply from up to 32x with crisp-edge your artwork before download, more impressively if it empathize no more than 128 colors, also by 6x creating very cool near-human shapes which the application reads to save resulting shapes by colors in a file that scales up to employ every living pixel on screen / printing unit on paper that aren't disabled.`)}} className={classes.link}>&nbsp;5)&nbsp;+Render*&nbsp;</a></Fade>,
        ];
