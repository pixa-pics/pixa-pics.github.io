...
class InnerToolbar extends React.Component {
    ...
    render() {
        ...
        const tip_items = [
            ...
            <Fade in={true}><a key="tip-smooth" onClick={() => {this._trigger_tip(`For a pixel art to get smooth, your pixel should live with similar neighbors in towns that isn't out-thought fancy by needs of pixel's house living the thug life within towns out of their identification, it is possible to move away many cheaters into thin air by comparing all-height other pixel's neighbors id (pixel town can be quite messy with noise or artefact caused by their irregularity of position) and counting up to six similar id against the pixel being judged within or outside of majority to execute its removal, algorithmic raid often create very few residual outsiders consequently. Double-tap or Right-click around the movable and drawing card area to open the context-menu containing the related tools.`)}} className={classes.link}>&nbsp;+smooth&nbsp;</a></Fade>,
        ];
