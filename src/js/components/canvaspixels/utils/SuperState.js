    ...
    from: function(props){
        ...
        return {
            ...
            set_state: function(new_props, is_large_object) {
                ...
                var sc =  this.set_cursor;
                return new Promise(function (resolve){
                    "use strict";
                    var update_cursor = false;
                    if("tool" in new_props) {if(state_.tool == new_props.tool) {update_cursor = true;}}
                    if("select_mode" in new_props) {if(state_.select_mode == new_props.select_mode) {update_cursor = true;}}
            ...
            set_notifiers: function(callback_function_update = function (){}) {

                notifiers = {
                    update: callback_function_update
                };
            },
            set_cursor: function( mouse_down, canvas_event_target) {
                "use strict";

                var tool = state_.tool;
                var select_mode = state_.select_mode;
                var _is_image_import_mode = state_._is_image_import_mode;
                var _is_on_resize_element = state_._is_on_resize_element;


                state_.mouse_down = mouse_down;
                state_.canvas_event_target = canvas_event_target;

                var old_cursor = ""+state_.cursor;

                if(_is_image_import_mode) {

                    if(_is_on_resize_element) {

                        state_.cursor = `url("${images_[0]}") 18 18, auto`
                    }else if(!mouse_down) {

                        state_.cursor =  `url("${images_[1]}") 20 18, auto`;
                    }else {

                        state_.cursor =  `url("${images_[2]}") 20 24, auto`;
                    }
                    ...
                    state_.cursor =  "grabbing";
                }else if (tool == "RECTANGLE") {

                    state_.cursor =  'url("'+images_[3]+'") 27 12, auto';
                }else if (tool == "ELLIPSE") {

                    state_.cursor =  'url("'+images_[4]+'") 25 9, auto';
                }else if (tool == "LINE") {

                    state_.cursor =  'url("'+images_[5]+'") 32 32, auto';
                }else if (tool == "PENCIL") {

                    state_.cursor =  'url("'+images_[6]+'") 5 32, auto';
                }else if (tool == "PENCIL PERFECT") {

                    state_.cursor =  'url("'+images_[7]+'") 5 32, auto';
                }else if (tool == "PICKER") {

                    state_.cursor =  'url("'+images_[8]+'") 5 32, auto';
                }else if (tool == "EXCHANGE") {

                    state_.cursor =  'url("'+images_[9]+'") 18 18, auto';
                }else if (tool == "BUCKET") {

                    state_.cursor =  'url("'+images_[10]+'") 6 25, auto';
                }else if (tool == "HUE BUCKET") {

                    state_.cursor =  'url("'+images_[11]+'") 6 25, auto';
                }else if (tool == "CONTOUR") {

                    state_.cursor =  'url("'+images_[12]+'") 8 33, auto';
                }else if (tool == "BORDER") {

                    state_.cursor =  'url("'+images_[13]+'") 18 18, auto';
                }else if (tool == "SET PENCIL MIRROR") {

                    state_.cursor =  'url("'+images_[14]+'") 18 18, auto';
                }else if (tool == "SELECT COLOR THRESHOLD") {

                    state_.cursor =  'url("'+images_[15]+'") 7 7, auto';
                }else if (tool == "SELECT PATH") {

                    state_.cursor =  'url("'+images_[16]+'") 8 33, auto';
                }else if (tool == "SELECT PIXEL") {

                    if(select_mode == "ADD" || select_mode == "REPLACE") {

                        state_.cursor =  'url("'+images_[17]+'") 5 32, auto';
                    }else if(select_mode == "REMOVE"){

                        state_.cursor =  'url("data:image/png;base64,") 5 32, auto';
                    }else if(select_mode == "REPLACE") {

                        state_.cursor =  'url("data:image/png;base64,") 18 18, auto';
                    }
                }else if (tool == "SELECT PIXEL PERFECT") {

                    if(select_mode == "ADD") {

                        state_.cursor =  'url("data:image/png;base64,") 5 32, auto';
                    }else if(select_mode == "REMOVE"){

                        state_.cursor =  'url("data:image/png;base64,") 5 32, auto';
                    }else if(select_mode == "REPLACE"){

                        state_.cursor =  'url("data:image/png;base64,") 5 32, auto';
                    }
                }else if(tool.includes("SELECT")) {

                    if(select_mode == "ADD") {

                        state_.cursor =  'url("data:image/png;base64,") 18 18, auto';
                    }else if(select_mode == "REMOVE") {

                        state_.cursor =  'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACQAAAAkCAYAAADhAJiYAAAAb0lEQVRYR+2UQQrAIBDEdv7/6C0+oAYWClriNbiOcTB12MphecpA9CIa0hAZIG6HNEQGiNshDZEB4tsOdXfTgAlP8nouBtrsnWSpdcd/BRppgE1jQ1+EoZl+jBoiA8TtkIbIAHE7pCEyQNwOXWfoAYpLFiWYvOCxAAAAAElFTkSuQmCC") 18 18, auto';
                    }else if(select_mode == "REPLACE") {

                        state_.cursor =  'url("data:image/png;base64,") 18 18, auto';
                    }
                }else {
            ...
            create_shape: function() {
                ...
                return {
                    ...
                    from_text: function (size, text, onto) {
                        ...
                        context.font = `${size}px "Industry Book"`;
