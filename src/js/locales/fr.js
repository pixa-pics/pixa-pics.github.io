module.exports = {
    _plurals: {
        "key": {
            "one": "clé",
            "many": "clés"
        }
    },
    words: {
        "unknown": "inconnu",
        "error": "erreur",
        "ok": "ok",
        "message": "message",
        "account": "compte",
        "accounts": "comptes",
        "acronym": "acronyme",
        "address": "adresse",
        "close": "fermer",
        "cancel": "annuler",
        "copy": "copier",
        "confirmation": "confirmation",
        "name": "nom",
        "password": "mot de passe",
        "open": "ouvrir",
        "loading": "chargement",
        "new": "nouveau",
        "next": "suivant",
        "refresh": "rafraîchir",
        "top up": "Aj. des fonds",
        "random": "aléatoire",
        "backup": "sauvegarde",
        "delete": "supprimer",
        "today": "aujourd'hui",
        "show": "montrer",
        "balance": "fonds",
        "transactions": "transactions",
        "charts": "graphiques",
        "send": "envoyer",
        "receive": "reçevoir",
        "settings": "paramètres",
        "share": "partager",
        "reset": "à zéro",
        "24h": "24h",
        "7d": "7j",
        "30d": "30j",
        "180d": "180j",
        "1y": "1an",
        "max": "max",
        "price": "prix",
        "cap": "cap",
        "market cap": "Cap. boursière",
        "public key": "clé publique",
        "private key": "clé privée",
        "crypto id": "ID de crypto",
        "total supply": "nombre émit",
        "all-time high": "le plus haut",
        "all-time low": "le plus bas",
        "homepages": "accueil",
        "block explorers": "explorateur de bloques",
        "forum": "forum",
        "chat": "chat",
        "feelings": "sentiments",
        "development score": "score de dev.",
        "average score": "score moyen",
        "liquidity": "liquidité",
        "confirm": "confirmer",
        "encrypt": "encrypter",
        "decrypt": "décrypter",
        "send at": "envoyer à",
        "send back": "envoyer en retour",
        "send from": "envoyé de",
        "send from public key": "envoyé de la clé publique",
        "send to public key": "envoyer à la clé publique",
        "send to": "envoyer à",
        "send again": "envoyer à nouveau",
        "memo": "mémo",
        "amount": "montant",
        "fee": "frais",
        "learn": "apprendre",
        "url": "url",
        "link": "lien",
        "locales": "locales",
        "currency": "monnaie",
        "create": "créer",
        "chart": "graphique",
        "qr code": "code QR",
        "scan": "analyser",
        "download": "télécharger",
        "yes": "oui",
    },
    sentences: {
        "your coins": "Vos pièces",
        "your previous coins": "Vos anciennes piècess",
        "random coins": "Pièces aléatoires",
        "invalid date message": "Date invalide",
        "new account": "Nouveau compte",
        "open an account": "Ouvrir un compte",
        "incorrect address": "Adresse incorrecte",
        "incorrect amount": "Montant incorrect",
        "incorrect message": "Message incorrect",
        "confirm transaction": "Confirmer la transaction",
        "send this transaction": "Envoyer cette transaction ?",
        "loading": "Chargement...",
        "load more": "Charger plus",
        "pick a date": "Choisir une date",
        "stay logged": "Rester connecté(e)",
        "market cap rank": "Rang de cap. boursière",
        "all accounts": "Tout les comptes",
        "alexa rank": "Rang Alexa",
        "today score": "Score 24h",
        "no balance": "Pas de fonds",
        "account name cannot be empty": "Le nom d'un compte ne peut pas être vide",
        "account name": "Nom du compte",
        "wrong password": "Mot de passe erroné",
        "wrong password input": "Entrée du mot de passe erroné",
        "wrong password confirmation": "Confirmation du mot de passe erronée",
        "something is incorrect": "Quelque chose est incorrecte",
        "bip39 mnemonic": "mnémonique bip39",
        "create a new account": "Créer un nouveau compte",
        "address successfully copied": "Adresse copiée avec succès",
        "url successfully copied": "Url copiée avec succès",
        "text successfully copied": "Texte copié avec succès",
        "public key successfully copied": "Clé publique copiée avec succès",
        "cannot copy this address": "Impossible de copier cette adresse",
        "cannot copy this url": "Impossible de copier cette url",
        "cannot copy this text": "Impossible de copier ce texte",
        "cannot copy this public key": "Impossible de copier cette clé publique",
        "cannot copy non-existent url": "Impossible de copier une url non-existente",
        "cannot copy non-existent text": "Impossible de copier un text non-existent",
        "cannot copy a non-existent address": "Impossible de copier une adresse non-existente",
        "cannot load more transaction": "Impossible de charger plus de transactions...",
        "cannot copy a non-existent public key": "Impossible de copier une clé publique qui n'existe pas",
        "copy address": "Copier l'adresse",
        "copy public key": "Copy la clé publique",
        "the longer you look the shiner i get": "Plus tu me regardes, plus je deviens éblouissant",
        "take a picture it last longer": "Prends une photo, ça dure plus longtemps",
        "stop bitchslapping me": "Arrête de m'agresser comme une baltringue",
        "hey i am jamy": "Salut, je suis Jamy.",
        "you need to add fund to this account": "Vous devez ajouter des fonds à ce compte",
        "just do it through the link in the menu": "Faites-le simplement au travers du lien dans le menu",
        "no transactions maid": "Vous n'avez pas encore fait de transactions, mais elles seront affichées ici.",
        "no transactions maid chart": "Vous n'avez pas encore fait de transactions, mais elles seront affichées ici dans un graphique.",
        "no transaction maid portfolio": "Vous n'avez pas encore fait de transactions, par conséquent, votre portfolio est vide et ne sera donc pas affiché.",
        "cannot load QR code scanner": "Impossible de charger le module d'analyse de QR code",
        "transaction sent": "Transaction envoyée",
        "scan an address": "Lire une adresse",
        "online": "Nous sommes en ligne !",
        "offline": "Nous sommes hors ligne !",
        "you must open an account": "Vous devez ouvir un compte.",
        "use a few words avoid common phrases": "Utilisez quelques mots, évitez les phrases courantes.",
        "no need for symbols digits or uppercase letters": "Pas besoin de symboles, de chiffres ou de lettres majuscules",
        "add another word or two uncommon words are better": "Ajoutez un mot ou deux. Les mots peu communs sont meilleurs.",
        "straight rows of keys are easy to guess": "Les rangées droites de touches sont faciles à deviner.",
        "short keyboard patterns are easy to guess": "Les modèles typographiques de clavier courts sont faciles à deviner.",
        "use a longer keyboard pattern with more turns": "Utilisez un motif de clavier plus long avec plus de possibilités.",
        "repeats like aaa are easy to guess": "Les répétitions comme aaa sont faciles à deviner.",
        "repeats like abcabcabc are only slightly harder to guess than abc": "Les répétitions comme \"abcabcabc \" ne sont que légèrement plus difficiles à deviner que \"abc\".",
        "avoid repeated words and characters": "Évitez les mots et les caractères répétés.",
        "sequences like abc or 6543 are easy to guess": "Des séquences comme \"abc\" ou \"6543\" sont faciles à deviner.",
        "avoid sequences": "Éviter les séquences.",
        "recent years are easy to guess": "Les dernières nouvelles années sont faciles à deviner.",
        "avoid recent years": "Éviter les années recentes.",
        "avoid years that are associated with you": "Évitez les années qui vous sont associées.",
        "dates are often easy to guess": "Les dates sont souvent faciles à deviner.",
        "avoid dates and years that are associated with you": "Évitez les dates et les années qui vous sont associées.",
        "this is a top-10 common password": "Ceci est un des 10 premiers mots de passe courants.",
        "this is a top-100 common password": "Ceci est un mot de passe commun du top 100.",
        "this is a very common password": "C'est un mot de passe très courant.",
        "this is similar to a commonly used password": "Ceci est similaire à un mot de passe couramment utilisé.",
        "a word by itself is easy to guess": "Un mot en lui-même est facile à deviner.",
        "names and surnames by themselves are easy to guess": "Les noms et prénoms par eux-mêmes sont faciles à deviner.",
        "common names and surnames are easy to guess": "Les noms et prénoms communs sont faciles à deviner.",
        "capitalization doesnt help very much": "La majuscule n'aide pas beaucoup.",
        "all-uppercase is almost as easy to guess as all-lowercase": "Tout en majuscule est presque aussi facile à deviner que tout en minuscule.",
        "reversed words arent much harder to guess": "Les mots inversés ne sont pas beaucoup plus difficiles à deviner.",
        "predictable substitutions like @ instead of a dont help very much": "Les substitutions prévisibles comme \"@\" au lieu de \"a\" n'aident pas beaucoup.",
    },
    components: {
        drawer_content: {
            dont_show_again: "Ne plus l'afficher",
            go: "J'y vais",
            menu: {
                more: {
                    contributors: "Contributeurs",
                    source_code: "Code Source",
                    more: "Plus",
                    download: "Télécharger",
                    donations: "Donations",
                    feedback: "Avis",
                },
            }
        },
        inner_toolbar: {
            guest: "Invité",
        },
        share_dialog: {
            title: "Partager",
            url: "Url",
            thanks_for_sharing: "Super, merci pour le partage !",
        },
    },
    pages: {
        index: {
            first_snackbar: "Portefeuille multi-crypto-monnaie en ReactJS",
            second_snackbar: "Open-source, gratuitement, pour tout le monde, pour toujours",
        },
        home: {
            ready_to_start_cta: "Prêt à commencer maintenant ? Créez un nouveau portefeuille anonyme !",
        },
        settings: {
            settings_changed: "Paramètres modifiés",
            language: "Langue",
            currency: "Devise",
            sound: "Son",
            enable_sound_effects: "Activer les sons",
            superintendent: "Général de surveillance",
            enable_the_superintendent: "Activer le général de surveillance",
            make_jamy_active: "Rendre Jamy actif",
            description_of_jamy: "Jamy est responsable de la surveillance, du jugement et de la réaction de vos actions en tant qu'utilisateur. Il ne peut dire à personne ce que vous faites, mais il est présent si vous le lui autorisez.",
            security: "Securité",
            enable_reset_option_in_menu: "Activer l'option de réinitialisation dans menu",
            fees: "Frais de transaction"
        }
    },
};